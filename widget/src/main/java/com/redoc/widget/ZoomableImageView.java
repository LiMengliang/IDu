...
/**
 * ZoomableImageView手势检测的流程：
 * 1. Set onTouchListener
 * 2. 在onTouchListern内处理Action_Down, Action_Move事件 （设置一下动作的flag）
 * 3. 在onTouchListener处理完Action之后，调用gestureDetector检测手势
 * 4. gestureDetector关联到一个GestureListener，在这个listener中检测到手势，并调用相应的手势处理函数
 */
public class ZoomableImageView extends ImageView{
    ...
    public class MatrixTouchListener implements OnTouchListener{
        ...
        private void checkDragable() {
            ...
            // 图片右边缘离开右边界，表示不可左移
            // mImageWidth 图片原始宽度
            // (mImageWidth)*values[Matrix.MSCALE_X] 当前缩放后的宽度， 也是图片图片右边缘
            // 相对于image控件左边缘的位置
            // (mImageWidth)*values[Matrix.MSCALE_X]+values[Matrix.MTRANS_X] xtranslate后图片右边缘
            // 相对于image控件左边缘的位置
            // getWidth() image控件宽度，也是image右边缘相对于image左边缘的位置
        ...
        public void onDoubleClick(){
            float startScale = isZoomChanged() ? mDobleClickScale : 1;
            float endScale = isZoomChanged() ? 1 : mDobleClickScale;
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    mCurrentMatrix.postScale(currentScale, currentScale, getWidth() / 2, getHeight() / 2);
                    setImageMatrix(mCurrentMatrix);
                }
            });
        }
        ...
        public void onFling(float velocityX, float velocityY) {
            final float endTranslateX = velocityX * 0.1f;
            final float endTranslateY = velocityY * 0.1f;
            ValueAnimator animator = ValueAnimator.ofFloat(0, 0, endTranslateX, endTranslateY);
            animator.setTarget(this);
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    float xTranslate = fraction * endTranslateX - lastX;
                    float yTranslate = fraction * endTranslateY - lastY;
                    lastX = fraction * endTranslateX;
                    lastY = fraction * endTranslateY;
                    float[] values=new float[9];
                    mCurrentMatrix.getValues(values);
                    yTranslate=checkDyBound(values, yTranslate);
                    xTranslate=checkDxBound(values,xTranslate,xTranslate);

                    mCurrentMatrix.postTranslate(xTranslate, yTranslate);
                    setImageMatrix(mCurrentMatrix);
                }
            });
        }
